...
case class Log4jReporter(logger: Logger = Logger.getLogger("WL"), logLevel: Level = Level.INFO) extends Reporter {

  override def report(checkResult: CheckResult): Unit = {
    val date = new Date()
    for ((_, constraintResult) <- checkResult.constraintResults) {
...
object Log4jReporter {
  ...
  private def columnsToJsonArray(columns: Seq[(String, String)]) = JSONArray(
    columns.map {
      case (baseColumn, referenceColumn) =>
        JSONObject(
          Map(
  ...
  private[reporters] def constraintResultToJson[T <: Constraint](checkResult: CheckResult,
    ...
    val constraintType =
      constraintResult.constraint.getClass.getSimpleName.replace("$", "")
    ...
    val constraintSpecificFields = constraintResult match {
      case alwaysNullConstraintResult: AlwaysNullConstraintResult =>
        Map(
      ...
      case anyOfConstraintResult: AnyOfConstraintResult =>
        Map(
      ...
      case columnColumnConstraintResult: ColumnColumnConstraintResult =>
        Map(
      ...
      case conditionalColumnConstraintResult: ConditionalColumnConstraintResult =>
        Map(
      ...
      case dateFormatConstraintResult: DateFormatConstraintResult =>
        Map(
      ...
      case ForeignKeyConstraintResult(ForeignKeyConstraint(columns, ref), nonMatchingRows, _) =>
        Map(
      ...
      case functionalDependencyConstraintResult: FunctionalDependencyConstraintResult =>
        Map(
      ...
      case joinableConstraintResult: JoinableConstraintResult =>
        Map(
      ...
      case neverNullConstraintResult: NeverNullConstraintResult =>
        Map(
      ...
      case numberOfRowsConstraintResult: NumberOfRowsConstraintResult =>
        Map(
          "expected" -> numberOfRowsConstraintResult.constraint.expected.toString,
          "actual"   -> numberOfRowsConstraintResult.actual
        )
      case regexConstraintResult: RegexConstraintResult =>
        Map(
      ...
      case stringColumnConstraintResult: StringColumnConstraintResult =>
        Map(
      ...
      case typeConversionConstraintResult: TypeConversionConstraintResult =>
        Map(
      ...
      case uniqueKeyConstraint: UniqueKeyConstraintResult =>
        Map(
